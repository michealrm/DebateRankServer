...
public class PF extends Module {
	...
	public PF(ArrayList<TabroomEntryInfo> tournaments, WorkerPool manager, Datastore datastore, MongoDatabase db) {
		super(LogManager.getLogger(net.debaterank.server.modules.jot.PF.class), datastore, db);
	...
	private void enterTournament(Datastore datastore, BufferedInputStream iStream, Tournament t, int tourn_id, int event_id) throws IOException, JSONException {
		...
		HashMap<Integer, Pair<PFRound, Ballot>> ballots = new HashMap<>();
		...
		for(int i = 0;i<jsonBallot_score.length();i++) {
			try {
				...
				Pair<PFRound, Ballot> ballot = ballots.get(ballotID);
				if(ballot == null) {
		...
		ArrayList<PFRound> collBallots = new ArrayList<>();
		for(Pair<PFRound, Ballot> pair : ballots.values()) {
			PFRound round = pair.getLeft();
